...
export default function MyBids() {                                                                 //MyBids component to display the bids placed by the user
    ...
    const bid={                                                 //bid object to store the selected bid details
        id:selectedBid[2],
    ...
    const authentication = async () => {                                        //authentication function to check if the user is authenticated
    ...
    const loadMyBids = async () => {                                            //loadMyBids function to get the bids placed by the user
        //await authentication();
        myUserName = authState?.idToken?.claims.preferred_username;             //myUserName to store the username of the user
        
        const myBidList = await axios.get (`http://localhost:8080/api/MyBids/getMyBids/${myUserName}`);                 //API to get the bids placed by the user

        await myBidList.data.map(()=>{                                                                                  //mapping the data to get the required fields
            setMyBids(myBidList.data);                                                                                  //setting the myBids with the data
        });
    };


    const loadSelectedBid = async (index) => {                                                              //loadSelectedBid function to get the selected bid details
        setSelectedBid(myBids[index]);                                                                      //setting the selectedBid with the bid details
        
    }


    const updateBid = async () => {                                                                                 //updateBid function to update the bid details
        
        try{
            const update= await axios.put(`http://localhost:8080/api/MyBids/updateBid`,bid);                        //API to update the bid details
                            ...
                            <tbody className="table-body">
                                {
                                    myBids.map((bid,index)=>(
                                            ...
                                            <tr key={index} onClick={()=>{loadSelectedBid(index)}}>
                        ...
                        
                            {
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td colSpan={2}> 
                                            <textarea rows={4} className='labelValue textfield' value={selectedBid[4]} 
                                                onChange={(e)=>{
                                                    e.preventDefault()
                                                    setSelectedBid([selectedBid[0],selectedBid[1],selectedBid[2],selectedBid[3],e.target.value,selectedBid[5],selectedBid[6],selectedBid[7]])
                                    ...
                                    <tr>
                                        <td colSpan={3}>
                                            &nbsp;&nbsp;&nbsp;&nbsp;<button className="btn btn-success btn-sm" onClick={updateBid} >Update</button> &nbsp;&nbsp;
                                            <button className="btn btn-danger btn-sm" onClick={deleteBid} >Delete</button> 
